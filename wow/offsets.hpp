...
namespace distant::wow::offsets
{
	...
	{
		constexpr auto num_processed_messages = memory::offset<wow::uint>(0xC5D638);
		constexpr auto client_connection = wow::memory::offset<wow::memory::address>(0x00C79CE0);
		//constexpr auto C5D638
	}
	...
	namespace object_manager
	{
		...
		constexpr auto local_guid	= memory::offset<wow::guid>(0xC0);
		constexpr auto map_id		= memory::offset<wow::uint>(0xCC);

		constexpr auto movement_globals = memory::offset<memory::address>(0xD4);
	}

	namespace movement_controller
	{
		// revise default_controller
		constexpr auto default_controller = memory::offset<memory::address>(0x11D);
		constexpr auto current_controller = memory::offset<memory::address>(0x120);
	}
	...
	namespace object
	{
		...
		//constexpr auto disabled = memory::offset<bool>(0xB8); // was causing random bugs
		// I think this address is more consistent since in CGObject::Disable it explicitly 
		// sets this offset to 0.
		constexpr auto disabled = memory::offset<bool>(0xCA);
